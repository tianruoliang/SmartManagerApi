...
from app.api._parser import Parser, paginate_parser, paginate_schema_base
...
query_parser = Parser()
query_parser.add_argument('company_id', type=str, help='公司ID')
query_parser.add_argument('goods_id', type=str, help='货品ID')
...
class GoodsRecordList(Resource):
    ...
    @ns.expect(paginate_parser, query_parser)
    @ns.marshal_with(goods_record_paginate_schema)
    def get(self):
        """获取货品出/入库记录"""
        query = query_parser.parse_args(delete_none=True)
        page_info = paginate_parser.parse_args()
        return get_goods_record_list(page_info, query)
